...
const currentTheme = ref(localStorage.getItem('color-theme') || (window.matchMedia('(prefers-color-scheme: dark)').matches ? 'dark' : 'light'));
...
<template>
    <div>
        <button @click="toggleTheme"
            ...
            <svg v-if="currentTheme === 'dark'" class="w-3.5 h-3.5" aria-hidden="true"
            ...
            <svg v-else class="w-3.5 h-3.5" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor"
                viewBox="0 0 20 20">
                <path
